 ...
 */
class PS_Forum_Notifier_Mailer {

	/**
	 */
	static public function __setup() {
		// Log emails going out
		//add_filter( 'wp_mail', array( 'PS_Forum_Notifier_Mailer', 'log' ), 10, 5 );

		// Add scheduled action for delayed e-mails
		add_action( 'ps_forum_notifier_scheduled_email', array( 'PS_Forum_Notifier_Mailer', 'send_notification_email' ), 1, 1 );
	}
	...
	static public function log( $whatever ) {
		$file = fopen( '/tmp/ps-forum-notifier.log', 'a' );
	...
	static public function send_notification_email( $user_id, $mail_properties = array() ) {
		if( empty( $mail_properties ) || ! is_array( $mail_properties ) ) {
			$mail_properties = get_user_meta( $user_id, 'ps_forum_notifier_emails' );
		}
			...
			switch( substr( $action, 4, 5 ) ) {
				case 'topic' :
					...
					$messages[] = sprintf(
						...
						$author->display_name,
						psf_get_forum_title( $forum_id ),
						strip_tags( psf_get_topic_content( $topic_id ) ),
				...
				case 'reply' :
					...
					$messages[] = sprintf(
						...
						psf_get_reply_url( $reply_id )
					);
					break;

				case 'quote' :
					$link = psf_get_topic_permalink( $topic_id );
					$subject = sprintf(
		...
		if( wp_mail( $user->user_email, $subject, $message ) ) {
			delete_user_meta( $user_id, 'ps_forum_notifier_emails' );
		}
	}
